package com.wpa.wpa.dao;

import com.wpa.wpa.bo.AbstractBusinessObject;
...
public class GenericHibernateJpaDao implements GenericDao {
    ...
    public <ENTITY> List<ENTITY> getAllOrderedDesc(String property, Class<ENTITY> clazz) {
        return getEntityManager().createQuery("SELECT e FROM " + clazz.getSimpleName() + " e ORDER BY e." + property + " DESC").getResultList();
    }
    ...
    public <ENTITY> List<ENTITY> getAllOrderedAsc(String property, Class<ENTITY> clazz) {
        return getEntityManager().createQuery("SELECT e FROM " + clazz.getSimpleName() + " e ORDER BY e." + property + " ASC").getResultList();
    }
    
    /**
     * Vrati vsechny objekty serazene vzestupne dle dane property1 a property2
     *
     * @param property
     * @return
     */
    @SuppressWarnings("unchecked")
    public <ENTITY> List<ENTITY> getAllOrderedAsc(String property1, String property2, Class<ENTITY> clazz) {
        return getEntityManager().createQuery("SELECT e FROM " + clazz.getSimpleName() + " e ORDER BY e." + property1 + " ASC, e." + property2 + " ASC").getResultList();
    }
